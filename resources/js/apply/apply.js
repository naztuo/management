...
function findPage(n) {
    $.ajax({
        url: contextPath + "/apply/findPage.do",
        type: "post",
        data: {
            "Fenye": n
        },
...
function applylists(arry) {
    for (var i = 0; i < arry.length; i++) {
        str += "<tr>";
        str += "<td>" + (i + 1) + "</td>"
        str += "<td>" + arry[i]["title"] + "</td>";
        str += "<td>" + arry[i]["sortName"] + "</td>";
        str += "<td>" + arry[i]["author"] + "</td>";
        str += "<td>" + arry[i]["datas"] + "</td>";
        str += "<td>";
        str += "<a href='apply_update.html?result="+arry[i]["id"]+"' class='layui-btn layui-btn-xs'><i class='layui-icon layui-icon-edit'></i>修改</a>";
    ...
    $(".new_dels").click(function () {
        ...
        layer.open({
            ...
            yes: function (index, layero) {
                $.ajax({
                    url: contextPath+"/apply/delapply.do",
                    type: "post",
                    data: {
                        "result": ids,
                    },
                    success: function (m) {
                        ...
                        if (arry.state == 1) {
                            ...
                            setTimeout("location.href='apply.html'", 1000);
                        } else {
...
$('#btns').click(function () {
    ...
    if (tit.trim().length == 0) {
        ...
        location.href = "searchApply.html?stitle=" + tit;
    }
